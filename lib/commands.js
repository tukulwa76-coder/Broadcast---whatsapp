...
// Beda dari status biasa: pakai wrapper groupStatusMessageV2, dikirim lewat
// relayMessage (bukan sendMessage biasa) supaya WA memperlakukannya sebagai
// status yang nyantol ke grup tertentu, bukan status publik ke semua kontak.
...
async function sendGroupStatus(sock, groupJid, content) {
  ...
  let innerContent;
  if (content.text) {
    innerContent = { extendedTextMessage: { text: content.text } };
    ...
    const generated = await generateWAMessageContent(
      { [mediaType]: content[mediaType], caption: content.caption || "" },
      { upload: sock.waUploadToServer }
    );
    innerContent = generated;
  } else {
    throw new Error("Tipe konten tidak didukung untuk status grup (hanya teks/foto/video).");
  }
  ...
  const innerKey = Object.keys(innerContent)[0];
  innerContent[innerKey].contextInfo = {
    ...(innerContent[innerKey].contextInfo || {}),
    ...contextInfo,
  };
  ...
  return sock.relayMessage("status@broadcast", wrapped, {
    messageId: undefined,
    statusJidList: participants,
    additionalNodes: [{ tag: "meta", attrs: { is_status_for_group: "1" } }],
  });
}
